...
CREATE TABLE payment (
	...
	document_number BIGINT NOT NULL,
    status VARCHAR(100)NOT NULL,
	CONSTRAINT pk_payment PRIMARY KEY (payment_number,method_payment),
    ...
    CONSTRAINT fk_clie_paym FOREIGN KEY (document_number,initials) REFERENCES client(document_number,initials)on update cascade on delete restrict
);
...
CREATE TABLE product (
    ...
    CONSTRAINT fk_desc_prod FOREIGN KEY (code_description) REFERENCES description(code)on update cascade on delete restrict,
...
CREATE TABLE order_details (
	...
	CONSTRAINT uk_order_details UNIQUE (reference,document_number,initials,total_value),
    CONSTRAINT fk_prod_orde_deta FOREIGN KEY (reference) REFERENCES product(reference)on update cascade on delete restrict,
    CONSTRAINT fk_clie_orde_deta FOREIGN KEY (document_number,initials) REFERENCES client(document_number,initials)on update cascade on delete restrict
);
...
CREATE TABLE order_ (
	...
	method_payment VARCHAR(40)NOT NULL,
	code_order_details INTEGER NOT NULL,
    order_date TIMESTAMP NOT NULL,
    status VARCHAR(100)NOT NULL,
	CONSTRAINT pk_order PRIMARY KEY (order_number),
	CONSTRAINT uk_order UNIQUE (document_number,initials,payment_number,method_payment,code_order_details),
    CONSTRAINT fk_orde_deta_orde FOREIGN KEY (code_order_details) REFERENCES order_details(code)on update cascade on delete restrict,
	...
	CONSTRAINT fk_paym_orde FOREIGN KEY (payment_number,method_payment) REFERENCES payment(payment_number,method_payment)on update cascade on delete restrict
);
